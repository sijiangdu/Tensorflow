...
def BRNN_Wrapper(X, num_classes, n_hidden_units, forget_bias = 1.0,  name='Basic_LSTM'):

  with tf.name_scope(name):

    W = tf.Variable(tf.truncated_normal([n_hidden_units, num_classes],stddev=0.1))
    b = tf.Variable(tf.zeros([num_classes]))
    W2 = tf.Variable(tf.truncated_normal([n_hidden_units, num_classes],stddev=0.1))
    b2 = tf.Variable(tf.zeros([num_classes]))
    cell = tf.contrib.rnn.BasicLSTMCell(n_hidden_units, forget_bias = forget_bias)
    init_state = cell.zero_state(batch_size, dtype=tf.float32)
    cell2 = tf.contrib.rnn.BasicLSTMCell(n_hidden_units, forget_bias = forget_bias)
    init_state2 = cell2.zero_state(batch_size, dtype=tf.float32)
    outputs, output_states = tf.nn.bidirectional_dynamic_rnn(cell,cell2, X,sequence_length=[sample_size for i in range(batch_size)], initial_state_fw=init_state, initial_state_bw=init_state2, time_major=False)
    outputs = outputs[0]
    outputs = tf.unstack(tf.transpose(outputs, [1,0,2]))
    results = tf.matmul(outputs[-1], W) + b
    outputs2 = outputs[1]
    outputs2 = tf.unstack(tf.transpose(outputs, [1,0,2]))
    results2 = tf.matmul(outputs[-1], W2) + b2
...
def train():
    ...
    pred = BRNN_Wrapper(x, n_classes, n_cell, FLAGS.forget_bias)
    ...
    with tf.name_scope('training_epoch'):
        ...
        fig1.suptitle('Tensorflow Bidirectional RNN - MNIST Digit Recognizer')
        acc_all = [0.0,0.0,0.0]
        plt.draw()
        plt.pause(0.3)
          ...
          while not coord.should_stop():
              ...
              if step * batch_size > training_iters:
                ...
                if(submit_size%1000 == 0):
                  show_mnist(feed_d[x],digits, "Outputs to submission file: "+str(submit_size))
                continue                 
              ...
              if step % test_at == 0:
                  ...
                  plt.draw()
                  plt.savefig(FLAGS.result_dir+'/mnist_brnn_LSTM'+'.png')
                  plt.pause(0.3)
        ...
        plt.figure(fig1.number)
        plt.savefig(FLAGS.result_dir+'/mnist_brnn_LSTM'+result_str+'.png')
        train_writer.close()
